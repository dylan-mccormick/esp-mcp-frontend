...
const LLMContextProvider = ({ children }: { children: ReactNode }) => {
    ...
    return <LLMContext.Provider
            value={ (connectionStatus === "connected" && client)
                    ? { connect, connectionStatus: "connected", apiKey, maxTokens, model, client, setApiKey, setMaxTokens, setModel }
                    : { connect, connectionStatus: connectionStatus === "connected" ? "connecting" : connectionStatus, apiKey, maxTokens, model, setApiKey, setMaxTokens, setModel }
             }
        >
        { children }
    </LLMContext.Provider>
